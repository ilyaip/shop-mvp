/**
 * Theme preset data for ThemeSwitcher demo mode
 * These are client-side only presets for demonstration purposes
 * Actual theme settings are loaded from the backend API
 */

import type { FrontSettings, ThemePreset } from '@/shared/types/theme'

// Preset themes for demo/testing purposes only
const presetThemes: Record<ThemePreset, FrontSettings> = {
  light: {
    colors: {
      primary: '#4F46E5',
      primaryHover: '#4338CA',
...
/**
 * Get a preset theme for demo/testing purposes
 * Note: Actual theme settings should be loaded from the backend API
 */
...
export const getAvailablePresets = (): ThemePreset[] => {
  return Object.keys(presetThemes) as ThemePreset[]
}
